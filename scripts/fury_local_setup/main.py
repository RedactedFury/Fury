...
def SetupNewChain():
    ...
    subprocess.getstatusoutput("fury gentx cooluser 1000000000stake --chain-id test-1 --keyring-backend test")
    subprocess.getstatusoutput("fury collect-gentxs")
    print("chain initialization done ✔️")

    with open(f"{HOME_DIR}/.fury/config/genesis.json", "r") as jsonFile:
        data = json.load(jsonFile)

    data["app_state"]["gov"]["deposit_params"]["min_deposit"][0]["denom"] = "ufury"
    ...
    with open(f"{HOME_DIR}/.fury/config/genesis.json", "w") as jsonFile:
        json.dump(data, jsonFile)
    
    print("genesis update done ✔️")

    rpcConfig = toml.load(f"{HOME_DIR}/.fury/config/config.toml")
    rpcConfig["rpc"]["laddr"]="tcp://0.0.0.0:26657" 
    rpcConfig["rpc"]["cors_allowed_origins"]= ["*"]

    with open(f"{HOME_DIR}/.fury/config/config.toml",'w') as f:
        toml.dump(rpcConfig, f)
        f.close()
    
    print("RPC configurations done ✔️")
    
    lcdConfig = toml.load(f"{HOME_DIR}/.fury/config/app.toml")
    lcdConfig["api"]["enable"]=True 
    lcdConfig["api"]["enabled-unsafe-cors"]= True

    with open(f"{HOME_DIR}/.fury/config/app.toml",'w') as f:
...
def GetGenesisAccAddress():
    command = f"fury keys show {GENESIS_ACCOUNT_NAME} --keyring-backend test --output json"
...
def Vote(option):
    ...
    command = f"fury tx gov vote {latestPropID} {option} --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
...
def AddApp(name, shortName, minGovDeposit=0, govTimeInSeconds=0):
    command = f"""fury tx gov submit-proposal add-app {name} {shortName} {minGovDeposit} {govTimeInSeconds} --title "New App" --description "Adding new app on fury" --deposit 10000000ufury --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddAsset(name, denom, decimals=1, isOnChain=1, assetOraclePriceRequired=1, isCdpMintable=1):
    ...
    command = f"""fury tx gov submit-proposal add-assets --add-assets-file "{fileName}" --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddPair(assetID1, assetID2):
    command = f"""fury tx gov submit-proposal add-pairs {assetID1} {assetID2}  --title "New Pair" --description "Adding new pair" --deposit 10000000ufury --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def MintToken(appID, assetID):
    ...
    command = f"fury tx tokenmint tokenmint {appID} {assetID} --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddAssetInAppsAndVote(appID, assetID):
    jsonData = {
        ...
        "title" : "fdv",
        "description" : "dffd",
        "deposit" : "100000000ufury"
    }
    fileName = f"assetMap-{datetime.datetime.now()}.json"
    with open(fileName, "w") as jsonFile:
        json.dump(jsonData, jsonFile)

    command = f"""fury tx gov submit-proposal add-asset-in-app --add-asset-mapping-file "{fileName}" --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def CreateLiquidityPair(appID, baseCoinDenom, quoteCoinDenom):
    command = f"""fury tx gov submit-proposal create-liquidity-pair {appID} {baseCoinDenom} {quoteCoinDenom}  --title "New Liquidity Pair" --description "Adding new liquidity pair" --deposit 10000000ufury --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def CreateLiquidityPool(appID, pairID, depositCoins):
    command = f"fury tx liquidity create-pool {appID} {pairID} {depositCoins} --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --gas 5000000 --keyring-backend test -y"
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def GetContractAddress(codeID):
    command = f"fury q wasm list-contract-by-code {codeID} --output json"
    resp = json.loads(subprocess.getstatusoutput(command)[1])
    return resp['contracts'][0]

def GetLastContractCodeID():
    command = "fury q wasm list-code --output json"
    resp = json.loads(subprocess.getstatusoutput(command)[1])
...
def StoreAndIntantiateWasmContract():
    ...
    for index, contractData in enumerate(WASM_CONTRACTS):
        ...
        command = f"fury tx wasm store {contractData['contractPath']} --from {GENESIS_ACCOUNT_NAME}  --chain-id {CHAIN_ID} --gas 5000000 --gas-adjustment 1.3 --keyring-backend test  -y  --output json"
        output = subprocess.getstatusoutput(command)[1]
        output = json.loads(output)
        if int(output["code"]) != 0:
            print(output)
        ...
        command = f"""fury tx wasm instantiate {currentCodeID} '{json.dumps(contractData['initator'])}' --label "Instantiate {contractData['name']}" --no-admin --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --gas 5000000 --gas-adjustment 1.3 --keyring-backend test -y"""
        output = subprocess.getstatusoutput(command)[1]
        output = json.loads(output)
        if int(output["code"]) != 0:
            print(output)
...
def ExecuteWasmGovernanceProposal(contractAddress, proposalID):
    ...
    command = f"""fury tx wasm execute {contractAddress} '{json.dumps(execute)}' --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --gas 5000000 --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def ProposeWasmProposal(contractAddress, proposal, proposlID):
    command = f"""fury tx wasm execute {contractAddress}  '{json.dumps(proposal)}' --amount 100000000uharbor --from {GENESIS_ACCOUNT_NAME}  --chain-id {CHAIN_ID} --gas 5000000 --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddAssetRates(assetName, jsonData):
    ...
    command = f"""fury tx gov submit-proposal add-asset-rates-params --add-asset-rates-params-file '{fileName}' --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test --gas 5000000 -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddLendPool(jsonData):
    ...
    command = f"""fury tx gov submit-proposal add-lend-pool --add-lend-pool-file '{fileName}' --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test --gas 5000000 -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddLendPair(pairString, jsonData):
    ...
    command = f"""fury tx gov submit-proposal add-lend-pairs --add-lend-pair-file '{fileName}' --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test --gas 5000000 -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddLendAssetPairMapping(assetID, poolID, pairIDs):
    command = f"""fury tx gov submit-proposal add-asset-to-pair-mapping {assetID} {poolID} {','.join([str(i) for i in pairIDs])} --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --title "Lend Asset Pair Mapping" --description "Adding New Lend Asset To Pair Mapping" --deposit 100000000ufury --keyring-backend test -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddLendAuctionParamsAndVote():
    ...
    command = f"""fury tx gov submit-proposal add-auction-params --add-auction-params-file '{fileName}' --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test --gas 5000000 -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
