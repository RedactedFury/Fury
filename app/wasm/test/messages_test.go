...
import (
	...
	"github.com/redactedfury/fury/app/wasm"
...
func TestWhitelistAssetLocker(t *testing.T) {
	...
	fury, ctx := SetupCustomApp()
	AddAppAsset(fury, *ctx)
	querier := wasm.NewQueryPlugin(&fury.AssetKeeper,
		&fury.LockerKeeper,
		&fury.TokenmintKeeper,
		&fury.Rewardskeeper,
		&fury.CollectorKeeper,
		&fury.LiquidationKeeper,
		&fury.EsmKeeper,
		&fury.VaultKeeper,
		&fury.LendKeeper,
		&fury.LiquidityKeeper)
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := wasm.WhiteListAsset(fury.LockerKeeper, *ctx, actor.String(), tc.msg)
...
func TestAddMsgAddExtendedPairsVault(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgAddExtendedPairsVault(fury.AssetKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgSetCollectorLookupTable(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgSetCollectorLookupTable(fury.CollectorKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgSetAuctionMappingForApp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgSetAuctionMappingForApp(fury.CollectorKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgUpdateCollectorLookupTable(t *testing.T) {
	...
	fury, ctx := SetupCustomApp()
	AddPair(fury, *ctx)
	AddCollectorLookuptable(fury, *ctx)
	querier := wasm.NewQueryPlugin(&fury.AssetKeeper,
		&fury.LockerKeeper,
		&fury.TokenmintKeeper,
		&fury.Rewardskeeper,
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgUpdateCollectorLookupTable(fury.CollectorKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgUpdatePairsVault(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgUpdatePairsVault(fury.AssetKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgWhitelistAppIDLiquidation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgWhitelistAppIDLiquidation(fury.LiquidationKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgRemoveWhitelistAppIDLiquidation(t *testing.T) {
	...
	fury, ctx := SetupCustomApp()
	AddPair(fury, *ctx)
	WhitelistAppIDLiquidation(fury, *ctx)
	querier := wasm.NewQueryPlugin(&fury.AssetKeeper,
		&fury.LockerKeeper,
		&fury.TokenmintKeeper,
		&fury.Rewardskeeper,
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgRemoveWhitelistAppIDLiquidation(fury.LiquidationKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgAddAuctionParams(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgAddAuctionParams(fury.AuctionKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgBurnGovTokensForApp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := wasm.MsgBurnGovTokensForApp(fury.TokenmintKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
