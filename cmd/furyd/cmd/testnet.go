...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).

Note, strict routability for addresses is turned off in the config file.

Example:
	%s testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`, chain.AppBinary),
	...
	cmd.Flags().String(flagNodeDaemonHome, chain.AppBinary, "Home directory of the node's daemon configuration")
