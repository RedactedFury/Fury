...
func ProposalContents(bk types.BankKeeper, k keeper.Keeper) []simtypes.WeightedProposalContent {
	return []simtypes.WeightedProposalContent{
		simulation.NewWeightedProposalContent(
			OpWeightSimulateMarketMakerProposal,
			params.DefaultWeightMarketMakerProposal,
			SimulateMarketMakerProposal(bk, k),
		),
		simulation.NewWeightedProposalContent(
...
func SimulateMarketMakerProposal(bk types.BankKeeper, k keeper.Keeper) simtypes.ContentSimulatorFn {
	return func(r *rand.Rand, ctx sdk.Context, accs []simtypes.Account) simtypes.Content {
		...
		inclusions := []types.MarketMakerHandle{}
		distributions := []types.IncentiveDistribution{}
		exclusions := []types.MarketMakerHandle{}
		rejections := []types.MarketMakerHandle{}
		...
		for _, mm := range mms {
			if !mm.Eligible {
				...
				if simtypes.RandIntBetween(r, 0, 2) == 1 {
					inclusions = append(inclusions, types.MarketMakerHandle{
						Address: mm.Address,
						PairId:  mm.PairId,
					})
				} else {
					// rejection
					rejections = append(rejections, types.MarketMakerHandle{
						Address: mm.Address,
						PairId:  mm.PairId,
					})
				}
			...
			if mm.Eligible && simtypes.RandIntBetween(r, 0, 7) == 1 {
				exclusions = append(exclusions, types.MarketMakerHandle{
					Address: mm.Address,
					PairId:  mm.PairId,
				})
			}
		...
		err = keeper.HandleMarketMakerProposal(ctx, k, proposal)
		if err != nil {
			panic(err)
		}
		return nil
