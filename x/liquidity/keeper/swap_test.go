...
func (s *KeeperTestSuite) TestAccumulatedSwapFeeConversion() {
	...
	asset1 := s.CreateNewAsset("ASSETONE", "ufury", 1000000)
	...
	s.nextBlock()
	accumulatedSwapFee := s.getBalances(pair.GetSwapFeeCollectorAddress())
	s.Require().True(utils.ParseCoins("936000ufury,936000uharbor").IsEqual(accumulatedSwapFee))
	...
	_ = s.CreateNewLiquidityPool(appID1, pair.Id, creator, "1000000000000ufury,1000000000000uharbor")
	...
	s.Require().True(utils.ParseCoins("936000ufury,9uharbor").IsEqual(accumulatedSwapFee))

	// now execute the order placed in above block, swap order for 9 uharbor placed again in next block
	s.nextBlock()
	accumulatedSwapFee = s.getBalances(pair.GetSwapFeeCollectorAddress())
	s.Require().True(utils.ParseCoins("1859952ufury").IsEqual(accumulatedSwapFee))

	// now execute the order placed in above block, this block will execute the order for 9 harbor placed above
	s.nextBlock()
	accumulatedSwapFee = s.getBalances(pair.GetSwapFeeCollectorAddress())
	s.Require().True(utils.ParseCoins("1871844ufury").IsEqual(accumulatedSwapFee))

	// now execute the order placed in above block, here 1uharbor is refunded back since it is very small amount for swap order.
	// here all harbor tokens are converted into fury, since fury is the default distribution token for rewards
	s.nextBlock()
	accumulatedSwapFee = s.getBalances(pair.GetSwapFeeCollectorAddress())
	s.Require().True(utils.ParseCoins("1871996ufury,1uharbor").IsEqual(accumulatedSwapFee))
}
