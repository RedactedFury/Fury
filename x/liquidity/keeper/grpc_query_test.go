	...
	for _, tc := range testCases {
		s.Run(tc.Name, func() {
			...
			if tc.ExpErr != nil {
				...
				s.Require().Equal(resp.PoolIncentives[0].TotalRewards, utils.ParseCoin("0ufury"))
				s.Require().Equal(resp.PoolIncentives[0].DistributedRewards, utils.ParseCoin("0ufury"))
