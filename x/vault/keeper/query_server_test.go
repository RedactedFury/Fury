...
func (s *KeeperTestSuite) TestQueryVaultInfoOfOwnerByApp() {
	...
	newVaults := []struct {
		...
		{
			SkipVaultCreation: true,
			Req:               &types.QueryVaultInfoOfOwnerByAppRequest{AppId: appID2, Owner: "fury..."},
...
func (s *KeeperTestSuite) TestQueryAllVaultIdsByAnOwner() {
	...
	newVaults := []struct {
		...
		{
			SkipVaultCreation: true,
			Req:               &types.QueryAllVaultIdsByAnOwnerRequest{Owner: "fury..."},
			ExpErr:            status.Errorf(codes.NotFound, "Address is not correct"),
			ExpResp:           nil,
		},
		{
...
func (s *KeeperTestSuite) TestQueryStableVaultByVaultID() {
	...
	asseTwoID := s.CreateNewAsset("ASSETTWO", "uasset2", 2000000)
	pairID := s.CreateNewPair(addr1, asseOneID, asseTwoID)
	extendedVaultPairID1 := s.CreateNewExtendedVaultPair("FURY-C", appID1, pairID, true, true)
	extendedVaultPairID2 := s.CreateNewExtendedVaultPair("FURY-C", appID2, pairID, true, true)
...
func (s *KeeperTestSuite) TestQueryUserMyPositionByApp() {
	...
	newVaults := []struct {
		...
		{
			SkipVaultCreation: true,
			Req:               &types.QueryUserMyPositionByAppRequest{AppId: appID1, Owner: "fury..."},
			ExpErr:            status.Errorf(codes.NotFound, "Address is not correct"),
			ExpResp:           nil,
		},
		{
