...
func (s *KeeperTestSuite) TestAddAssetRecords() {
	...
	for _, tc := range []struct {
		...
		{
			"Add Asset : Duplicate Denom 2 osmo ufury",
			assetTypes.Asset{
				Name:                  "OSMO",
				Denom:                 "ufury",
				Decimals:              sdk.NewInt(1000000),
...
func (s *KeeperTestSuite) TestWasmUpdatePairsVault() {
	...
	for _, tc := range []struct {
		...
		{
			"Update Extended Pair Vault : fury fust",
...
func (s *KeeperTestSuite) TestAddExtendedPairVault() {
	...
	for _, tc := range []struct {
		...
		{
			"Add Extended Pair Vault : Duplicate PairName fust fury",
